class ProfileUpdateService
  ...
  def self.recalibrate(profile)
    ...
    amounts = transactions.pluck(:amount).map(&:to_f)
    profile.average_transaction_amount = amounts.empty? ? 0 : (amounts.sum / amounts.size.to_f)
    profile.max_transaction_amount_seen = amounts.max || 0
